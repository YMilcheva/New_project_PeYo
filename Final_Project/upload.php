							...
							if ($_FILES["song_file"]["type"] != 'audio/mpeg'){
								...
								if (move_uploaded_file($_FILES['song_file']['tmp_name'], $uploadfile) == false) {
							     echo '<h3 class="text-center text-secondary">There was an error with handling your song. Please contact us from the form below!</h3>';
							 		...
							 		$insert_query = "INSERT INTO songs (song_path, song_name, song_singer, song_date, song_uploader, song_soft_delete, song_genre) VALUES ('" . $uploadfile . "', '" . trim(htmlentities($_POST['song_name'])). "', " . $singer_validation[$_POST['singer_name']] . ",'" . date("Y-m-d H:i:s") . "', " . $_SESSION['user_id'] . ", NULL, " . $genre_validation[$_POST['genre_name']] . ")";
									$result = mysqli_query($conn, $insert_query); 
									if ($result) { 
										echo '<h3 class="text-center text-secondary">Your song is uploaded!</h3>';					
									} else {
										echo '<h3 class="text-center text-secondary">Error: ' . mysqli_error($conn) . '</h3>';
									}
							 	}
